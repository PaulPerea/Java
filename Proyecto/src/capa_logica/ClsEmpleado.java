...
public class ClsEmpleado {
	...
	public int RegistrarEmpleado(Empleado emp) {
		...
		try {
			...
			sql="insert into EMPLEADO values(?,?,?,?,?,?)"; //? este signo es el parametro por defecto
			...
			pstm.setString(2, emp.getApe());
			pstm.setString(3, emp.getDni());
			pstm.setString(4, emp.getCargo());
	...
	public int ActualizarEmpleado(Empleado emp) {
		...
		try {
			...
			cone=MySQLConec.miconexion();
			//segundo preaprar la sentencia insert
			sql="update EMPLEADO set nom_emp=?,ape_emp=?,dni_emp=?,cargo_emp=?,edad_emp=? WHERE  id_emp=?"; //? este signo es el parametro por defecto
			//tercero vamos a ejectar el sql  a travez de pstm
			pstm=cone.prepareStatement(sql);
			//cuarto, realizar el seteo de datos a cada parametro
			pstm.setString(1, emp.getNom()); // osea que inserta al primer campo de categoria
			pstm.setString(2, emp.getApe());
			pstm.setString(3, emp.getDni());
			pstm.setString(4, emp.getCargo());
			pstm.setString(5, emp.getEdad());
	...
	public int EliminarEmpleado(Empleado emp) {
		...
		try {
			...
			cone=MySQLConec.miconexion();
			//segundo preaprar la sentencia insert
			sql="delete from EMPLEADO where  id_emp=?"; //? este signo es el parametro por defecto
			//tercero vamos a ejectar el sql  a travez de pstm
			pstm=cone.prepareStatement(sql);
			//cuarto, realizar el seteo de datos a cada parametro
	...
	public void ListadoEmpleado(DefaultTableModel modelo, String[] datos, String consulta) {
